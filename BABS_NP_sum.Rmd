...
Note: Inconsistent number of participants across conditions.
...
Note: Also, need to look up the range of acceptable scores to verify values.
...
d.physAct <- BABS.NP.sess1 %>% select(BABS_ID, `AGE AT SESSION`, starts_with("PHYSICAL_ACT")) %>%
                                          distinct(BABS_ID, .keep_all = T) %>%
  mutate_at(vars(starts_with("PHYSICAL_ACT")), as.numeric)
#counting the number of observations and summarizing mean & SD
physAct.sum <- d.physAct %>% describe() %>% select(-vars) %>% rownames_to_column(var = "Variable") %>%
  filter(Variable != "BABS_ID*") %>% filter(Variable != "AGE AT SESSION")

kable(physAct.sum, col.names = c("Variable","N","Mean","SD","Median","Trimmed Mean","Median Abs. Deviation","Min","Max","Range","Skew","Kurtosis","SE"),
      caption = "Physical Activity Summary Table") %>% kable_styling()
```

## Depressive and Anxious Traits {.tabset}
### Geriatric Depression Questionnaire (GDS)
Allowable range is 0-30 (integers only). The following participants have scores that are not integers: BABS106 (10.5), BABS111 (0.5), BABS114 (3.5), BABS118 (7.5).
...
Subscores range from 10-50 (integers only). The following participants have scores that are not integers: BABS070 (MASQ_GD = 13.5, MASQ_AD = 26.5), BABS072 (MASQ_GD = 16.5), BABS111 (MASQ_AA = 14.5).
...
Scores range from 16-80 (integers only). The following participants have scores that are not integers: BABS029 (50.13), BABS038 (21.9375), BABS041(39.31), BABS045 (55.47), BABS083 (29.87), BABS092 (45.87).
...
BABS284 (PSQI_DISTB = 12; range=0-3). Integer values only acceptable; BABS057 (PSQI_SLPQUAL = 1.5), BABS159 (PSQI_SLPQUAL = 1.5).
...
This instrument has not been scored.
...
Note: Scores should be integers only (General range: 18-90; Ability range: 20-100; Strategy range: 19-95). Table printed below with values not entered as integers.

Following scores are out of range (_GENERAL): BABS112, BABS134, BABS151, BABS132, BABS133, BABS152
Following scores are out of range (_STRATEGIES): BABS152
...
Note: Scores should be integers only (Total range : 32-224; WM range: 8-56; Initiate range: 8-56; PO range: 10-70; TM range: 6-42). The following values were entered as non-integers: BABS007 (BRIEF_WM = 14.5, BRIEF_TOTAL = 46.5); BABS014 (BRIEF_PO = 20.5, BRIEF_TOTAL = 77.5), BABS171 (BRIEF_TM = 9.5, BRIEF_TOTAL = 42.5). 

Following scores are out of range: BABS284(0s are not an acceptable value; multiple subscales), BABS260 (BRIEF_WM = 72, BRIEF_PO = 71, BRIEF_TM = 57, BRIEF_TOTAL = 36), BABS264 (BRIEF_WM = 65, BRIEF_PO = 77, BRIEF_TM = 75, BRIEF_TOTAL = 28), BABS199 (BRIEF_WM = 62, BRIEF_PO = 73, BRIEF_TM = 75, BRIEF_TOTAL = 28).

It appears that in instances when there were no observed rawscore, a scores of 0 was entered for "_TOTAL"; should double-check that the following participants do not have any raw scores and then remove 0s from data frame for "_TOTAL"

The following participants have a "BRIEF_TOTAL" == 0:  "BABS010" "BABS022" "BABS070" "BABS107" "BABS120" "BABS165" "BABS168" "BABS174" "BABS175" "BABS176" "BABS177" "BABS178" "BABS179" "BABS180" "BABS181" "BABS182" "BABS183" "BABS184" "BABS185" "BABS186" "BABS187" "BABS188" "BABS189" "BABS190" "BABS191" "BABS192" "BABS193" "BABS194" "BABS195" "BABS196" "BABS197" "BABS198" "BABS200"
...
Scores should be integers only (each subscale ranges from 0-12; total= 0-48). There are many instances of non-integer values, so printing a data frame with the incorrect values & subject IDs below.
...
Note: Missing the scoring of the first-order BIS scales.
...
### Grit Scale
Note: Range of "GRIT_SCORE" can only range from 1 to 5; it appears that in instances when there was no observed "GRIT_RAW" score, a score of 0 was entered for "GRIT_SCORE"; should double-check that the following participants do not have any Grit scores and then remove 0s from data frame for "GRIT_SCORE"

The following participants have a "GRIT_SCORE" == 0: "BABS118" "BABS120" "BABS121" "BABS174" "BABS175" "BABS176" "BABS177" "BABS178" "BABS179" "BABS180" "BABS181" "BABS182" "BABS183" "BABS184" "BABS185" "BABS186" "BABS187" "BABS188" "BABS189" "BABS190" "BABS191" "BABS192"
...
Note: It appears that in instances when there was no observed "_RAW" score, a score of 0 was entered for "_SCORE"; should double-check that the following participants do not have any raw scores and then remove 0s from data frame for "_SCORE" variables

The following participants have a "_SCORE" == 0: "BABS010" "BABS082" "BABS107" "BABS120" "BABS121" "BABS124" "BABS127" "BABS153" "BABS163" "BABS165" "BABS167" "BABS168" "BABS169" "BABS170" "BABS172" "BABS174" "BABS175" "BABS176" "BABS177" "BABS178" "BABS179" "BABS180" "BABS181" "BABS182" "BABS183" "BABS184" "BABS185" "BABS186" "BABS187" "BABS188" "BABS189" "BABS190" "BABS191" "BABS192"
